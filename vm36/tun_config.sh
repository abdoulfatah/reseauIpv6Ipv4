...
#sudo ip addr add fc00:1234::10/64 dev tun0
#sudo ip link set tun0 up

# Creation d'une route par le tunnel pour atteindre l'autre réseau
#sudo ip -6 route add fc00:1234::1/64 dev "$1"
#                    \              /
#                   adresse a atteindre
...
#sudo ip -6 route add fc00:1234:1::/64 via fc00:1234:ffff::10 dev "$1"



#sudo /sbin/ifconfig "$1" inet add 172.16.2.1/28

#sudo ip link set "$1" up

#sudo ip route add 172.16.2.176 via 172.16.2.1 dev "$1"

sudo ip addr add 172.16.2.1/28 dev $1
sudo ip link set $1 up
sudo ip route add 172.16.2.144/28 via 172.16.2.1 dev $1
